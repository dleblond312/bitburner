...
let WEAK_SCRIPT = "weaken.js";
let TOR_SCRIPT = "tor.js";
let HOME_UPGRADE_SCRIPT = "home.js";
let PURCHASE_SCRIPT = "purchase.js";
let PURCHASE_SERVER_SCRIPT = "purchase_server.js";
let FACTION_SCRIPT = "factions.js";
let AUGMENT_SCRIPT = "augment.js";
let MONITOR_SCRIPT = "monitor.js";
let TRAVEL_SCRIPT = "travel.js";
let STOCK_SCRIPT = "stock.js";
let STOCK_PURCHASE_SCRIPT = "stock_upgrade.js";
let BACKDOOR_SCRIPT = "backdoor.js";

let EXIT_SERVER = "w0r1d_d43m0n";
let EVENT_PORT = 1;
let SCHEDULING_PORT = 2;
let STOP_SCALING_PORT = 6;
let PURCHASE_SERVER_PORT = 7;
let NULL_PORT_DATA = "NULL PORT DATA";


let total_scheduled_threads = {};
let time_functions = {};
...
let date_format = Intl.DateTimeFormat('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', fractionalSecondDigits: 3 });
var currency_format = Intl.NumberFormat('en', { notation: 'compact', maximumSignificantDigits: 4, style: 'currency', currency: 'USD' });
...
export class NodeInfo {
	 ...
	 */
	constructor(isRooted, isOwned, hostname, total_ram, can_schedule, threads, available_money, delta_security_level, max_money) {
...
export class Operation {
	 ...
	 */
	constructor(script, target, hostname, threads, end_time) {
		this.script = script;
		this.target = target;
		this.hostname = hostname;
		this.threads = threads;
...
export async function main(ns) {
	...
	time_functions = {};
	time_functions[HACK_SCRIPT] = ns.getHackTime;
	time_functions[WEAK_SCRIPT] = ns.getWeakenTime;
	time_functions[GROW_SCRIPT] = ns.getGrowTime;

	security_impact_functions = {};
	security_impact_functions[HACK_SCRIPT] = ns.hackAnalyzeSecurity;
	security_impact_functions[WEAK_SCRIPT] = null;
	security_impact_functions[GROW_SCRIPT] = ns.growthAnalyzeSecurity;
	...
	do {
		...
		let total_tracked = {};
		total_tracked[HACK_SCRIPT] = 0;
		total_tracked[WEAK_SCRIPT] = 0;
		total_tracked[GROW_SCRIPT] = 0;

		total_scheduled_threads = {};
		total_scheduled_threads[HACK_SCRIPT] = 0;
		total_scheduled_threads[WEAK_SCRIPT] = 0;
		total_scheduled_threads[GROW_SCRIPT] = 0;
		...
		let hosts = await crawl_network(ns, "home", {}, 0);

		// Generate maximum port count we can compromise
		let port_count = programs.reduce((accumulator, program) => {
		...
		for (var i = 0; i < hosts.length; i++) {
			let host = hosts[i];
			var server_info = analyze_server(ns, host);
		...
		if (ns.getDarkwebPrograms().length > 0) {
			for(var i = 0; i < programs.length; i++) {
				let program = programs[i];
				if (!ns.fileExists(program.name, "home") && ns.singularity.getDarkwebProgramCost(program.name) < ns.getPlayer().money && !ns.scriptRunning(PURCHASE_SCRIPT, "home")) {
		...
		if (ns.getUpgradeHomeRamCost() < ns.getPlayer().money) {
		...
		operations.forEach((operation) => {
			let node = op_list.filter((node) => { return node.hostname == operation.target });
			if (node.length > 0 && Number.isFinite(operation.threads)) {
				node[0].threads[operation.script] = Math.max(0, node[0].threads[operation.script] - operation.threads);
 ...
 */
async function scale_out_nodes(ns, rooted_list) {
	if (ns.getPurchasedServerLimit() > pservers.length) {
		...
		let target_ram = max_pserv_ram > ns.getServerMaxRam(min_pserv) * 2 ? max_pserv_ram : ns.getServerMaxRam(min_pserv) * 2;
		if (ns.getServerMaxRam("home") > 32 && target_ram <= ns.getPurchasedServerMaxRam() && ns.getPurchasedServerCost(target_ram) <= ns.getPlayer().money) {
			if (ns.isRunning(HACK_SCRIPT, min_pserv) || ns.isRunning(WEAK_SCRIPT, min_pserv) || ns.isRunning(GROW_SCRIPT, min_pserv)) {
				var msg = `Draining server: ${min_pserv}`;
...
*/
async function crawl_network(ns, node, node_list, depth) {
	let hosts = ns.scan(node);
	for (var i = 0; i < hosts.length; i++) {
		var host = hosts[i];
		if (!node_list[host]) {
			node_list[host] = true;
 ...
 * @returns {NodeInfo}
 */
function analyze_server(ns, host) {
	let hack_threads_required = 0;
	let grow_threads_required = 0;

	let isRooted = ns.getServer(host).hasAdminRights;

	var total_ram = ns.getServer(host).maxRam;
	let isOwned = is_owned(ns, host);

	var max_money = ns.getServer(host).moneyMax;
	var available_money = ns.getServer(host).moneyAvailable;
	var available_money_ratio = max_money ? available_money / max_money : 0;
	var delta_security_level = calc_security_delta(ns, host);
	var weak_threads_required = Math.ceil(Math.max(delta_security_level / 0.05, 0));

	if (!isOwned && available_money > 0 && delta_security_level < 2) {
		var grow_multiplier = 1 / available_money_ratio;
	...
	let threads = {};
	threads[WEAK_SCRIPT] = weak_threads_required;
	threads[HACK_SCRIPT] = hack_threads_required;
	threads[GROW_SCRIPT] = grow_threads_required;

	var info = new NodeInfo(isRooted, isOwned, host, total_ram, true, threads, available_money, delta_security_level, max_money);
	return info;
}
 ...
 * @param {NS} ns
 */
function calc_security_delta(ns, host) {
	var security_level = ns.getServerSecurityLevel(host);
	var min_security_level = ns.getServerMinSecurityLevel(host);
	var delta_security_level = security_level - min_security_level;
 ...
 */
async function schedule_breach(ns, breach_list) {
	while (breach_list.length > 0) {
		let host = breach_list.shift();
 ...
 */
async function schedule_script(ns, scripts, rooted_list, target_list) {
	...
	while (target_list.length > 0 && rooted_list.length > 0) {
		...
		let available_memory = source.total_ram - ns.getServerUsedRam(source.hostname);
		let available_threads = Math.floor(available_memory / script_ram);
		switch (script_name) {
			case WEAK_SCRIPT:
		...
		if (num_threads > 0) {
			if (ns.exec(script_name, source.hostname, num_threads, target.hostname, Date.now(), num_threads)) {
				...
				if (security_impact_functions[script_name]) {
					let security_from_script = security_impact_functions[script_name](num_threads);
					let weakened_threads_added = Math.ceil(Math.max(security_from_script / 0.05, 0));
